...
# =========================
# 数据根目录：程序会读取 BASE_DIR/original，并输出 cropped、corrected、workflow_result。
BASE_DIR = r"D:\RT-dPCR IMG\1210-4"
...
# ROI 裁剪坐标，格式为 (x, y)，分别是左上角和右下角。
ROI_TOP_LEFT = (1418, 1386)
ROI_BOTTOM_RIGHT = (3300, 2933)

# 稳定图像起点：排序后的前两张有偏移，所以默认从第 3 张开始提取坐标和画曲线。
STABLE_START_INDEX = 3

# Stage1/Stage2 指定图名。留空时自动使用稳定区间的第 1、2 张，即总序列第 3、4 张。
STAGE1_IMAGE_NAME = ""
STAGE2_IMAGE_NAME = ""

# 终点阴阳性判读方法："kmeans" 为自动聚类，"threshold" 为手动阈值。
ENDPOINT_METHOD = "kmeans"

# threshold 方法参数：POSITIVE_THRESHOLD 为阈值，THRESHOLD_IS_NORMALIZED=True 表示用 0-1 归一化强度判读。
POSITIVE_THRESHOLD = 0.25
THRESHOLD_IS_NORMALIZED = True

# 微孔识别参数：面积范围、圆度阈值、采样半径。
MIN_AREA = 20
MAX_AREA = 150
MIN_CIRCULARITY = 0.65
RADIUS = 3

# 两阶段筛点参数：Stage1 去除亮度异常孔，Stage2 去除荧光保留率过低的孔。
STD_MULTIPLIER = 2.0
MIN_RETENTION_RATIO = 0.85

# 终点判读时剔除异常暗孔的 MAD 系数，越小剔除越严格。
DARK_OUTLIER_MAD_FACTOR = 3.5

# kmeans 判读中对阴性孔扣除背景的偏移量，通常保持 0。
INTENSITY_OFFSET = 0.0

# 单独阴性/阳性曲线图的高斯平滑窗口。越大越平滑，但过大会抹平 S 型拐点。
CURVE_SMOOTH_WINDOW = 5

# 曲线 baseline 周期数：用于计算 F0，delta=F-F0，normalized=(F-F0)/F0。
CURVE_BASELINE_CYCLES = 5

# 曲线是否只使用稳定区间图像。True 会跳过前两张偏移图。
CURVE_USE_STABLE_RANGE_ONLY = True

# 单独曲线图纵轴范围/留白参数。POSITIVE_CURVE_Y_TOP <= 0 表示阳性曲线自动上限。
CURVE_Y_TOP = 0.30
CURVE_Y_BOTTOM = -0.15
POSITIVE_CURVE_Y_TOP = 0.0
CURVE_HEADROOM_RATIO = 0.20

# 曲线纵轴数值模式："delta" 推荐；"raw" 为原始灰度；"normalized" 为 (F-F0)/F0。
# CURVE_VALUE_MODE = "normalized"
CURVE_VALUE_MODE = "delta"
# CURVE_VALUE_MODE = "raw"

# 曲线离群识别 MAD 系数。越小，黄色/蓝色离群曲线越多。
CURVE_OUTLIER_MAD_FACTOR = 3.5

# 曲线图画布大小。宽度越小，横轴视觉上越短。
CURVE_FIG_WIDTH = 9.0
CURVE_FIG_HEIGHT = 7.0

# 合并图是否隐藏离群曲线。True 隐藏黄色/蓝色离群曲线；False 显示出来。
COMBINED_HIDE_OUTLIER_CURVES = True

# 合并图中阳性曲线平滑窗口。想让红色阳性曲线更顺滑，就调大这个值，例如 7、9、11。
POSITIVE_COMBINED_SMOOTH_WINDOW = 7

# 合并图中阴性曲线额外平滑窗口。越大，灰色阴性曲线越顺滑。
NEGATIVE_COMBINED_SMOOTH_WINDOW = 15

# 合并图中阴性曲线显示收束比例。1=不收束，0=全部压到中位数曲线，越小灰色簇越紧。
NEGATIVE_COMBINED_SHRINK_FACTOR = 0.35

# =========================
# Effective configuration / 有效配置区
# 下面这一段是程序实际使用的配置。前面的旧注释区如果显示乱码，不影响这里。
# =========================
BASE_DIR = r"D:\RT-dPCR IMG\1210-4"
ORIGINAL_DIR = os.path.join(BASE_DIR, "original")
CROPPED_DIR = os.path.join(BASE_DIR, "cropped")
...
# 终点判读方法："kmeans" 自动聚类；"threshold" 手动阈值。
ENDPOINT_METHOD = "kmeans"
POSITIVE_THRESHOLD = 0.25
THRESHOLD_IS_NORMALIZED = True
...
# 字体参数：所有图使用 Times New Roman；这里控制每块文字大小和粗细。
...
def extract_well_curve_matrix(corrected_paths, wells):
    raw_matrix = np.zeros((len(wells), len(corrected_paths)), dtype=np.float32)
    for image_index, image_path in enumerate(corrected_paths):
        ...
        for well_index, (cx, cy) in enumerate(wells):
            ...
            raw_matrix[well_index, image_index] = float(np.mean(image[y_start:y_end, x_start:x_end]))
...
def select_curve_matrix(raw_matrix):
    ...
    if CURVE_VALUE_MODE == "raw":
        return raw_matrix, delta_matrix, normalized_matrix, raw_matrix, "Green gray value"
    if CURVE_VALUE_MODE == "delta":
        return raw_matrix, delta_matrix, normalized_matrix, delta_matrix, "Green gray value change"
    if CURVE_VALUE_MODE == "normalized":
        return raw_matrix, delta_matrix, normalized_matrix, normalized_matrix, "Normalized F"
...
def save_well_curve_csv(path, corrected_paths, wells, raw_matrix, delta_matrix, normalized_matrix, smoothed_matrix):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(
...
def plot_classified_well_curves(corrected_paths, wells, label, line_alpha, y_top, y_bottom):
    ...
    raw_matrix = extract_well_curve_matrix(corrected_paths, wells)
    raw_matrix, delta_matrix, normalized_matrix, plot_matrix, y_label = select_curve_matrix(raw_matrix)
    ...
    save_well_curve_csv(csv_path, corrected_paths, wells, raw_matrix, delta_matrix, normalized_matrix, smoothed_matrix)
    ...
    y_min = float(np.percentile(smoothed_matrix, 0.5))
    y_max = float(np.percentile(smoothed_matrix, 99.5))
    ...
    ax.set_ylim(min(y_bottom, auto_bottom), top_limit)
...
def save_combined_curve_summary(path, negative_wells, positive_wells, neg_distances, pos_distances, neg_outliers, pos_outliers):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(["group", "well_id", "x", "y", "curve_distance", "is_curve_outlier"])
        for index, ((cx, cy), distance, is_outlier) in enumerate(
            zip(negative_wells, neg_distances, neg_outliers),
            start=1,
        ):
            writer.writerow(["negative", index, cx, cy, float(distance), bool(is_outlier)])
        for index, ((cx, cy), distance, is_outlier) in enumerate(
            zip(positive_wells, pos_distances, pos_outliers),
            start=1,
        ):
            writer.writerow(["positive", index, cx, cy, float(distance), bool(is_outlier)])
...
def plot_combined_well_curves(corrected_paths, negative_wells, positive_wells):
    ...
    if negative_wells:
        negative_raw = extract_well_curve_matrix(corrected_paths, negative_wells)
        _, _, _, negative_plot_matrix, y_label = select_curve_matrix(negative_raw)
        negative_smoothed = smooth_curve_matrix(negative_plot_matrix, CURVE_SMOOTH_WINDOW)
    if positive_wells:
        positive_raw = extract_well_curve_matrix(corrected_paths, positive_wells)
        _, _, _, positive_plot_matrix, y_label = select_curve_matrix(positive_raw)
    ...
    if negative_display.size:
        ...
        non_outlier_mask = ~negative_outliers
    ...
    for curve, is_outlier in zip(positive_smoothed, positive_outliers):
        if not is_outlier:
            ax.plot(cycles, curve, color="#E31A1C", alpha=0.58, linewidth=1.05, solid_capstyle="round", zorder=2)

    for curve, is_outlier in zip(negative_display, negative_outliers):
        if is_outlier and not COMBINED_HIDE_OUTLIER_CURVES:
    ...
    all_curves = []
    if negative_display.size:
        negative_ylim_matrix = negative_display if not COMBINED_HIDE_OUTLIER_CURVES else negative_display[~negative_outliers]
        if negative_ylim_matrix.size:
            all_curves.append(negative_ylim_matrix)
    if positive_smoothed.size:
        positive_ylim_matrix = positive_smoothed if not COMBINED_HIDE_OUTLIER_CURVES else positive_smoothed[~positive_outliers]
    ...
    y_min = float(np.percentile(all_smoothed, 0.5))
    y_max = float(np.percentile(all_smoothed, 99.5))
    y_span = max(0.05, y_max - y_min)
    auto_bottom = y_min - max(0.02, y_span * 0.18)
    auto_top = y_max + max(0.03, y_span * CURVE_HEADROOM_RATIO)

    ax.set_title("dPCR smoothed curves", fontsize=PLOT_TITLE_FONTSIZE, fontweight=PLOT_FONT_WEIGHT, pad=10)
    ax.set_xlabel("Cycle", fontsize=PLOT_LABEL_FONTSIZE, fontweight=PLOT_FONT_WEIGHT)
    ax.set_ylabel(y_label, fontsize=PLOT_LABEL_FONTSIZE, fontweight=PLOT_FONT_WEIGHT)
    ax.set_xlim(1, max(1, len(corrected_paths)))
    ax.set_ylim(min(CURVE_Y_BOTTOM, auto_bottom), auto_top)
    ...
    print(f"    Positive display smooth window: {POSITIVE_COMBINED_SMOOTH_WINDOW}")
...
def parse_args():
    ...
    parser.add_argument(
        "--curve-value-mode",
        choices=["normalized", "delta", "raw"],
        default=CURVE_VALUE_MODE,
        help="Curve y-value mode: normalized=DeltaF/F0, delta=gray value change, raw=corrected gray value.",
    )
    parser.add_argument(
