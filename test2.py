...
def classify_endpoint_fluorescence_single(roi_gray_corrected, roi_color, valid_wells, radius=3):
    ...
    # 散点图 (模拟目标统计图)
    plt.subplot(1, 2, 1)
    plt.title('Endpoint Fluorescence Statistical Chart')
    partition_numbers = np.arange(len(valid_wells))
    plt.scatter(partition_numbers, norm_end_intensities, c=scatter_colors, alpha=0.5, s=5)
    plt.axhline(norm_threshold, color='black', linestyle='dashed', linewidth=2,
...
if __name__ == "__main__":
    ...
    IMAGE_PATH = r"D:\RT-dPCR IMG\dPCR-YR\test\9-3-10（2.jpg"

    ROI_TOP_LEFT = (910, 634)
