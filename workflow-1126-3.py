...
# Configuration
# =========================
BASE_DIR = r"D:\RT-dPCR IMG\1126-night"
...
STABLE_START_INDEX = 3
STAGE1_IMAGE_NAME = ""
STAGE2_IMAGE_NAME = ""
MASK_SOURCE = "endpoint"
...
def crop_original_images(original_dir, cropped_dir, top_left, bottom_right):
    ...
    for image_path in image_files:
        ...
        x2, y2 = bottom_right if bottom_right is not None else (width, height)
...
def endpoint_mask_filter(cropped_paths, stable_start_index=3):
    stable_paths = get_stable_paths(cropped_paths, stable_start_index)
    endpoint_path = stable_paths[-1]
    endpoint_gray = cv2.cvtColor(read_image_bgr(endpoint_path), cv2.COLOR_BGR2GRAY)
    endpoint_map = read_image_bgr(endpoint_path)

    candidates = detect_stage1_candidates(endpoint_gray)
    final_wells = [(cx, cy) for cx, cy, _, _, _ in candidates]
    for cx, cy in final_wells:
        cv2.circle(endpoint_map, (cx, cy), RADIUS, (0, 255, 0), 1)

    os.makedirs(RESULT_DIR, exist_ok=True)
    write_image(os.path.join(RESULT_DIR, "endpoint_mask_wells.jpg"), endpoint_map)
    save_wells_csv(os.path.join(RESULT_DIR, "valid_wells.csv"), final_wells)

    print("[2] Endpoint mask filter finished")
    print(f"    Stable range starts at sorted image #{stable_start_index}")
    print(f"    Endpoint mask image: {os.path.basename(endpoint_path)}")
    print(f"    Morphology candidates: {len(candidates)}")
    print(f"    Final valid wells: {len(final_wells)}")
    return final_wells
...
def classify_by_kmeans(endpoint_gray, endpoint_color, valid_wells):
    ...
    adjusted_intensities = raw_intensities.copy()
...
def save_classification_outputs(result):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as handle:
        ...
        for index, ((cx, cy), raw_i, norm_i, is_positive) in enumerate(
            ...
            writer.writerow([index, cx, cy, float(raw_i), float(norm_i), "positive" if is_positive else "negative"])
...
def early_max_zero_curve_matrix(raw_matrix, baseline_cycles):
    ...
    baseline_end = max(1, min(int(baseline_cycles), raw_matrix.shape[1]))
    early_max = np.max(raw_matrix[:, :baseline_end], axis=1, keepdims=True)
...
def flatten_negative_raw_curves(curve_matrix):
    if CURVE_VALUE_MODE != "raw" or curve_matrix.size == 0 or NEGATIVE_RAW_FLATTEN_FACTOR <= 0:
        return curve_matrix

    flatten_factor = float(np.clip(NEGATIVE_RAW_FLATTEN_FACTOR, 0.0, 1.0))
    center_curve = np.median(curve_matrix, axis=0, keepdims=True)
    shared_trend = center_curve - center_curve[:, :1]
    return (curve_matrix - flatten_factor * shared_trend).astype(np.float32)
...
def plot_classified_well_curves(corrected_paths, wells, label, line_alpha, y_top, y_bottom):
    ...
    if label == "negative":
        smoothed_matrix = flatten_negative_raw_curves(smoothed_matrix)
...
def save_combined_curve_summary(
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(
            ["group", "well_id", "x", "y", "curve_distance", "is_curve_outlier", "is_early_cycle_outlier", "is_hidden"]
        )
        for index, ((cx, cy), distance, is_outlier, is_early_outlier) in enumerate(
            zip(negative_wells, neg_distances, neg_outliers, neg_early_outliers),
            start=1,
        ):
            writer.writerow(
                ["negative", index, cx, cy, float(distance), bool(is_outlier), bool(is_early_outlier), bool(is_outlier or is_early_outlier)]
            )
        for index, ((cx, cy), distance, is_outlier, is_early_outlier) in enumerate(
            ...
            writer.writerow(
                ["positive", index, cx, cy, float(distance), bool(is_outlier), bool(is_early_outlier), bool(is_outlier or is_early_outlier)]
            )
...
def plot_combined_well_curves(corrected_paths, negative_wells, positive_wells):
    ...
    if COMBINED_REZERO_AFTER_SMOOTH and CURVE_VALUE_MODE in {"delta", "normalized", "early_max_zero"}:
    ...
    negative_hidden_mask = negative_outliers | negative_early_outliers
    ...
    if negative_display.size:
        ...
        negative_display = flatten_negative_raw_curves(negative_display)
    ...
    print(f"    Negative raw flatten factor: {NEGATIVE_RAW_FLATTEN_FACTOR:.2f}")
    print(f"    Re-zero after smoothing: {COMBINED_REZERO_AFTER_SMOOTH}; cycles={COMBINED_REZERO_CYCLES}")
...
def run_workflow(method):
    ...
    if MASK_SOURCE == "endpoint":
        valid_wells = endpoint_mask_filter(cropped_paths, STABLE_START_INDEX)
    elif MASK_SOURCE == "two_stage":
...
def parse_args():
    ...
    parser.add_argument(
        "--mask-source",
        choices=["endpoint", "two_stage"],
        default=MASK_SOURCE,
        help="Where valid-well mask is detected from. endpoint uses the final stable image; two_stage uses v2 baseline retention.",
    )
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        "--negative-raw-flatten-factor",
        type=float,
        default=NEGATIVE_RAW_FLATTEN_FACTOR,
        help="Display-only flatten factor for raw negative curves. 0=no flatten, 1=remove the shared median negative trend.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Curve y-value mode: normalized=DeltaF/F0, delta=gray value change, raw=corrected gray value, sum_delta=integrated gray value change, early_max_zero=raw minus early-cycle maximum.",
    )
    parser.add_argument(
...
def resolve_base_dir(base_dir):
    if os.path.isdir(os.path.join(base_dir, "original")):
        return base_dir
    ...
    child_dirs = [path for path in child_dirs if os.path.isdir(os.path.join(path, "original"))]
    if len(child_dirs) == 1:
        return child_dirs[0]
    return base_dir
...
if __name__ == "__main__":
    ...
    NEGATIVE_RAW_FLATTEN_FACTOR = args.negative_raw_flatten_factor
