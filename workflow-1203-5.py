...
# Configuration
# =========================
BASE_DIR = r"D:\RT-dPCR IMG\1203-(-5)"
...
ROI_TOP_LEFT = (0, 0)
ROI_BOTTOM_RIGHT = (2119,1274)

STABLE_START_INDEX = 1
STAGE1_IMAGE_NAME = ""
STAGE2_IMAGE_NAME = ""

ENDPOINT_METHOD = "kmeans"
...
ENDPOINT_SPATIAL_CORRECTION = True
ENDPOINT_SPATIAL_GRID_COLS = 8
ENDPOINT_SPATIAL_GRID_ROWS = 6
ENDPOINT_SPATIAL_BACKGROUND_QUANTILE = 0.20
...
def remove_dark_outliers(raw_intensities, wells):
    if len(raw_intensities) == 0:
        return raw_intensities, wells, 0, 0.0
    ...
    neg_label_pre = 1 if centers_pre[0][0] > centers_pre[1][0] else 0
    neg_intensities = raw_intensities[labels_pre.flatten() == neg_label_pre]

    if len(neg_intensities) > 0:
        neg_median = float(np.median(neg_intensities))
        neg_mad = float(np.median(np.abs(neg_intensities - neg_median)))
    ...
    kept_wells = [well for well, keep in zip(wells, keep_mask) if keep]
    return raw_intensities[keep_mask], kept_wells, removed_count, dark_cutoff


def get_quadratic_spatial_features(wells, reference_wells=None):
    coords = np.asarray(wells, dtype=np.float32)
    reference_coords = coords if reference_wells is None else np.asarray(reference_wells, dtype=np.float32)
    x = coords[:, 0]
    y = coords[:, 1]
    x_center = float(np.mean(reference_coords[:, 0]))
    y_center = float(np.mean(reference_coords[:, 1]))
    x_scale = max(1.0, float(np.ptp(reference_coords[:, 0])) / 2.0)
    y_scale = max(1.0, float(np.ptp(reference_coords[:, 1])) / 2.0)
    xn = (x - x_center) / x_scale
    yn = (y - y_center) / y_scale
    return np.column_stack([np.ones_like(xn), xn, yn, xn * xn, yn * yn, xn * yn]).astype(np.float32)


def correct_endpoint_spatial_trend(raw_intensities, wells):
    if not ENDPOINT_SPATIAL_CORRECTION or len(raw_intensities) < 20:
        return raw_intensities.copy(), False
    ...
    col_indices = np.minimum(
        ((x - float(np.min(x))) / x_span * ENDPOINT_SPATIAL_GRID_COLS).astype(np.int32),
        ENDPOINT_SPATIAL_GRID_COLS - 1,
    )
    row_indices = np.minimum(
        ((y - float(np.min(y))) / y_span * ENDPOINT_SPATIAL_GRID_ROWS).astype(np.int32),
        ENDPOINT_SPATIAL_GRID_ROWS - 1,
    )

    grid_wells = []
    grid_backgrounds = []
    grid_weights = []
    for row in range(ENDPOINT_SPATIAL_GRID_ROWS):
        for col in range(ENDPOINT_SPATIAL_GRID_COLS):
            mask = (row_indices == row) & (col_indices == col)
            count = int(np.sum(mask))
            if count < 12:
                continue
            grid_wells.append((float(np.median(x[mask])), float(np.median(y[mask]))))
            grid_backgrounds.append(
                float(np.quantile(raw_intensities[mask], ENDPOINT_SPATIAL_BACKGROUND_QUANTILE))
            )
            grid_weights.append(np.sqrt(count))

    if len(grid_wells) < 6:
        return raw_intensities.copy(), False

    features = get_quadratic_spatial_features(wells)
    grid_features = get_quadratic_spatial_features(grid_wells, reference_wells=wells)
    weights = np.asarray(grid_weights, dtype=np.float32)
    try:
        coefficients, *_ = np.linalg.lstsq(
            grid_features * weights[:, None],
            np.asarray(grid_backgrounds, dtype=np.float32) * weights,
            rcond=None,
        )
    except np.linalg.LinAlgError:
        return raw_intensities.copy(), False

    background = features @ coefficients
    reference_background = float(np.median(background))
    corrected = raw_intensities - (background - reference_background)
    return corrected.astype(np.float32), True


def classify_by_kmeans(endpoint_gray, endpoint_color, valid_wells):
    raw_intensities, wells = sample_well_intensities(endpoint_gray, valid_wells)
    raw_intensities, wells, removed_count, dark_cutoff = remove_dark_outliers(raw_intensities, wells)
    if len(raw_intensities) < 2:
        raise ValueError("Not enough wells remain for kmeans classification.")

    adjusted_intensities, spatial_corrected = correct_endpoint_spatial_trend(raw_intensities, wells)
    if INTENSITY_OFFSET > 0:
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 0.2)
        _, labels_pre, centers_pre = cv2.kmeans(
            adjusted_intensities.reshape(-1, 1),
            2,
            None,
            criteria,
            10,
        ...
        neg_label_pre = 1 if centers_pre[0][0] > centers_pre[1][0] else 0
        neg_mask = labels_pre.flatten() == neg_label_pre
        adjusted_intensities[neg_mask] = np.maximum(0.0, adjusted_intensities[neg_mask] - INTENSITY_OFFSET)

    min_i = float(np.min(adjusted_intensities))
    max_i = float(np.max(adjusted_intensities))
    normalized = np.zeros_like(adjusted_intensities) if max_i == min_i else (adjusted_intensities - min_i) / (max_i - min_i)
    ...
    pos_label = 0 if centers[0][0] > centers[1][0] else 1
    threshold = float((centers[0][0] + centers[1][0]) / 2.0)
    positive_mask = labels.flatten() == pos_label
    ...
    return build_classification_result(
        ...
        classification_intensities=adjusted_intensities,
        normalized=normalized,
        positive_mask=positive_mask,
        threshold=threshold,
        removed_count=removed_count,
        dark_cutoff=dark_cutoff,
        spatial_corrected=spatial_corrected,
    )


def classify_by_threshold(endpoint_gray, endpoint_color, valid_wells, threshold, threshold_is_normalized):
    raw_intensities, wells = sample_well_intensities(endpoint_gray, valid_wells)
    raw_intensities, wells, removed_count, dark_cutoff = remove_dark_outliers(raw_intensities, wells)
    if len(raw_intensities) == 0:
        raise ValueError("No wells remain for threshold classification.")
    ...
    classify_values = normalized if threshold_is_normalized else raw_intensities
    ...
    return build_classification_result(
        ...
        wells=wells,
        raw_intensities=raw_intensities,
        classification_intensities=raw_intensities,
        normalized=normalized,
        positive_mask=positive_mask,
...
def build_classification_result(
    ...
    removed_count,
    dark_cutoff,
    spatial_corrected=False,
):
    result_map = endpoint_color.copy()
    positive_wells = []
    negative_wells = []

    for (cx, cy), is_positive in zip(wells, positive_mask):
        if is_positive:
    ...
    if n_pos == 0:
        ...
        lambda_value = float(-np.log(1 - n_pos / n_total))
    ...
    return {
        ...
        "raw_intensities": raw_intensities,
        "classification_intensities": classification_intensities,
        "normalized": normalized,
        "positive_mask": positive_mask,
        "threshold": threshold,
        "lambda": lambda_value,
        "removed_dark_count": removed_count,
        "dark_cutoff": dark_cutoff,
        "spatial_corrected": spatial_corrected,
...
def save_classification_outputs(result):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(["well_id", "x", "y", "raw_intensity", "classification_intensity", "normalized_intensity", "classification"])
        for index, ((cx, cy), raw_i, class_i, norm_i, is_positive) in enumerate(
            zip(
                result["wells"],
                result["raw_intensities"],
                result["classification_intensities"],
                result["normalized"],
                result["positive_mask"],
            ),
            start=1,
        ):
            writer.writerow([index, cx, cy, float(raw_i), float(class_i), float(norm_i), "positive" if is_positive else "negative"])
    ...
    print(f"[4] {method} endpoint classification finished")
    print(f"    Total valid wells: {len(result['wells'])}")
    print(f"    Negative wells: {len(result['negative_wells'])}")
    print(f"    Positive wells: {len(result['positive_wells'])}")
    print(f"    Threshold: {result['threshold']:.4f}")
    print(f"    Lambda: {result['lambda']:.4f}")
    print(f"    Removed dark wells: {result['removed_dark_count']} (cutoff={result['dark_cutoff']:.3f})")
    print(f"    Endpoint spatial correction: {result['spatial_corrected']}")
...
def early_max_zero_curve_matrix(raw_matrix, baseline_cycles):
    if raw_matrix.size == 0:
        return raw_matrix.copy()

    baseline_end = max(1, min(int(baseline_cycles), raw_matrix.shape[1]))
    offset = float(EARLY_MAX_ZERO_OFFSET) if EARLY_MAX_ZERO_OFFSET is not None else float(np.max(raw_matrix[:, :baseline_end]))
    return (raw_matrix - offset).astype(np.float32)
...
def get_curve_group_title(label):
    return f"real-time dPCR {label} curves"
...
def detect_late_cycle_drop_outliers(curve_matrix):
    count = curve_matrix.shape[1]
    baseline_end = max(1, min(count, LATE_DROP_BASELINE_CYCLES))
    if curve_matrix.shape[0] == 0 or baseline_end >= count:
        return np.zeros(curve_matrix.shape[0], dtype=bool), np.zeros(curve_matrix.shape[0], dtype=np.float32), 0.0

    baseline_means = np.mean(curve_matrix[:, :baseline_end], axis=1)
    scores = np.min(curve_matrix[:, baseline_end:], axis=1) - baseline_means
    return scores < 0.0, scores.astype(np.float32), 0.0


def select_mild_display_outlier_indices(display_matrix, hidden_mask, max_count):
    indices = np.flatnonzero(hidden_mask)
    visible_matrix = display_matrix[~hidden_mask]
    if indices.size == 0 or max_count <= 0 or visible_matrix.size == 0:
        return indices[:0]

    center_curve = np.median(visible_matrix, axis=0)
    lower_band = np.percentile(visible_matrix, 10, axis=0)
    upper_band = np.percentile(visible_matrix, 90, axis=0)
    band_span = max(1.0, float(np.median(upper_band - lower_band)))

    scores = []
    for index in indices:
        curve = display_matrix[index]
        outside_band = np.maximum.reduce([lower_band - curve, curve - upper_band, np.zeros_like(curve)])
        center_distance = float(np.mean(np.abs(curve - center_curve)) / band_span)
        band_excursion = float(np.max(outside_band) / band_span)
        scores.append(center_distance + 2.5 * band_excursion)
...
def plot_classified_well_curves(corrected_paths, wells, label, line_alpha, y_top, y_bottom):
    ...
    smooth_window = (
        max(CURVE_SMOOTH_WINDOW, POSITIVE_COMBINED_SMOOTH_WINDOW)
        if label == "positive"
        else CURVE_SMOOTH_WINDOW
    )
...
def plot_negative_well_curves(corrected_paths, negative_wells):
    plot_classified_well_curves(
        corrected_paths=corrected_paths,
        wells=negative_wells,
        label="negative",
        line_alpha=0.32,
        y_top=CURVE_Y_TOP,
...
def save_combined_curve_summary(
    path,
    negative_wells,
    positive_wells,
    neg_distances,
    ...
    neg_early_outliers,
    pos_early_outliers,
    neg_late_drop_outliers,
    neg_late_drop_scores,
):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.writer(handle)
        writer.writerow(
            ["group", "well_id", "x", "y", "curve_distance", "is_curve_outlier", "is_early_cycle_outlier", "late_drop_score", "is_late_drop_outlier", "is_hidden"]
        )
        for index, ((cx, cy), distance, is_outlier, is_early_outlier, late_drop_score, is_late_drop_outlier) in enumerate(
            zip(negative_wells, neg_distances, neg_outliers, neg_early_outliers, neg_late_drop_scores, neg_late_drop_outliers),
            start=1,
        ):
            writer.writerow(
                ["negative", index, cx, cy, float(distance), bool(is_outlier), bool(is_early_outlier), float(late_drop_score), bool(is_late_drop_outlier), bool(is_outlier or is_early_outlier or is_late_drop_outlier)]
            )
        for index, ((cx, cy), distance, is_outlier, is_early_outlier) in enumerate(
            zip(positive_wells, pos_distances, pos_outliers, pos_early_outliers),
            start=1,
        ):
            writer.writerow(
                ["positive", index, cx, cy, float(distance), bool(is_outlier), bool(is_early_outlier), "", False, bool(is_outlier or is_early_outlier)]
            )


def plot_combined_well_curves(corrected_paths, negative_wells, positive_wells):
    if not corrected_paths:
        print("[6] No corrected images available for combined curve plotting.")
        return
    if not negative_wells and not positive_wells:
        print("[6] No classified wells available for combined curve plotting.")
        return
    ...
    if COMBINED_REZERO_AFTER_SMOOTH and CURVE_VALUE_MODE in {"delta", "normalized"}:
        negative_smoothed = rebaseline_curve_matrix(negative_smoothed, COMBINED_REZERO_CYCLES)
        positive_smoothed = rebaseline_curve_matrix(positive_smoothed, COMBINED_REZERO_CYCLES)

    negative_outliers, negative_distances, negative_threshold = detect_curve_outliers(negative_smoothed)
    positive_outliers, positive_distances, positive_threshold = detect_curve_outliers(positive_smoothed)
    negative_early_outliers = np.zeros_like(negative_outliers)
    positive_early_outliers = np.zeros_like(positive_outliers)
    negative_late_drop_outliers = np.zeros_like(negative_outliers)
    negative_late_drop_scores = np.zeros(negative_smoothed.shape[0], dtype=np.float32)
    negative_early_threshold = 0.0
    positive_early_threshold = 0.0
    negative_late_drop_threshold = 0.0
    if EARLY_CYCLE_FILTER_ENABLED:
        negative_early_outliers, _, negative_early_threshold = detect_early_cycle_outliers(
            negative_smoothed,
            EARLY_CYCLE_COUNT,
            EARLY_CYCLE_OUTLIER_MAD_FACTOR,
        )
        positive_early_outliers, _, positive_early_threshold = detect_early_cycle_outliers(
            positive_smoothed,
            EARLY_CYCLE_COUNT,
            EARLY_CYCLE_OUTLIER_MAD_FACTOR,
        )
    if LATE_DROP_FILTER_ENABLED and negative_smoothed.size:
        late_drop_matrix = smooth_curve_matrix(
            negative_smoothed,
            max(CURVE_SMOOTH_WINDOW, NEGATIVE_COMBINED_SMOOTH_WINDOW),
        )
        negative_late_drop_outliers, negative_late_drop_scores, negative_late_drop_threshold = detect_late_cycle_drop_outliers(
            late_drop_matrix
        )
    negative_hidden_mask = negative_outliers | negative_early_outliers | negative_late_drop_outliers
    positive_hidden_mask = positive_outliers | positive_early_outliers

    negative_display = negative_smoothed.copy()
    if negative_display.size:
        smooth_window = max(CURVE_SMOOTH_WINDOW, NEGATIVE_COMBINED_SMOOTH_WINDOW)
        negative_display = smooth_curve_matrix(negative_display, smooth_window)
        if COMBINED_REZERO_AFTER_SMOOTH and CURVE_VALUE_MODE in {"delta", "normalized"}:
            negative_display = rebaseline_curve_matrix(negative_display, COMBINED_REZERO_CYCLES)
        non_outlier_mask = ~negative_hidden_mask
        if np.any(non_outlier_mask):
            negative_display[non_outlier_mask] = shrink_curves_toward_median(
                negative_display[non_outlier_mask],
                NEGATIVE_COMBINED_SHRINK_FACTOR,
            )

    positive_display = positive_smoothed.copy()
    if positive_display.size:
        positive_display = smooth_curve_matrix(
            positive_display,
            max(CURVE_SMOOTH_WINDOW, POSITIVE_COMBINED_SMOOTH_WINDOW),
        )
        if COMBINED_REZERO_AFTER_SMOOTH and CURVE_VALUE_MODE in {"delta", "normalized"}:
            positive_display = rebaseline_curve_matrix(positive_display, COMBINED_REZERO_CYCLES)
        non_outlier_mask = ~positive_hidden_mask
        if np.any(non_outlier_mask):
            positive_display[non_outlier_mask] = shrink_curves_toward_median(
                positive_display[non_outlier_mask],
                POSITIVE_COMBINED_SHRINK_FACTOR,
            )

    fig, ax = plt.subplots(figsize=(CURVE_FIG_WIDTH, CURVE_FIG_HEIGHT), dpi=220)

    for curve, is_hidden in zip(negative_display, negative_hidden_mask):
        if not is_hidden:
            ax.plot(cycles, curve, color="#B8B8B8", alpha=0.18, linewidth=0.55, solid_capstyle="round", zorder=1)

    for curve, is_hidden in zip(positive_display, positive_hidden_mask):
        if not is_hidden:
            ax.plot(cycles, curve, color="#E31A1C", alpha=0.58, linewidth=1.05, solid_capstyle="round", zorder=2)

    for curve, is_outlier in zip(negative_display, negative_outliers):
        if is_outlier and not COMBINED_HIDE_OUTLIER_CURVES and not EARLY_CYCLE_FILTER_ENABLED:
            ax.plot(cycles, curve, color="#8DD7FF", alpha=0.9, linewidth=1.25, solid_capstyle="round", zorder=4)

    for curve, is_outlier in zip(positive_display, positive_outliers):
        if is_outlier and not COMBINED_HIDE_OUTLIER_CURVES and not EARLY_CYCLE_FILTER_ENABLED:
            ax.plot(cycles, curve, color="#FFD900", alpha=0.95, linewidth=1.45, solid_capstyle="round", zorder=5)

    all_curves = []
    if negative_display.size:
        negative_ylim_matrix = negative_display[~negative_hidden_mask]
        if negative_ylim_matrix.size:
            all_curves.append(negative_ylim_matrix)
    if positive_display.size:
        positive_ylim_matrix = positive_display[~positive_hidden_mask]
        if positive_ylim_matrix.size:
            all_curves.append(positive_ylim_matrix)
    if not all_curves:
    ...
    ax.set_ylabel(y_label, fontsize=PLOT_LABEL_FONTSIZE, fontweight=PLOT_FONT_WEIGHT)
    ax.set_xlim(1, max(1, len(corrected_paths)))
    bottom_limit, top_limit = get_display_ylim(all_smoothed, CURVE_Y_BOTTOM, POSITIVE_CURVE_Y_TOP)
    ax.set_ylim(bottom_limit, top_limit)
    ...
    figure_path = os.path.join(RESULT_DIR, f"combined_well_smoothed_{CURVE_VALUE_MODE}_curves.png")
    representative_outlier_path = os.path.join(RESULT_DIR, f"combined_well_smoothed_{CURVE_VALUE_MODE}_curves_with_mild_outliers.png")
    summary_path = os.path.join(RESULT_DIR, "combined_curve_outliers.csv")
    plt.savefig(figure_path, bbox_inches="tight", dpi=300)
    plt.close(fig)

    mild_negative_indices = select_mild_display_outlier_indices(
        negative_display,
        negative_hidden_mask,
        max(1, REPRESENTATIVE_OUTLIER_CURVES // 2),
    )
    mild_positive_indices = select_mild_display_outlier_indices(
        positive_display,
        positive_hidden_mask,
        max(1, REPRESENTATIVE_OUTLIER_CURVES - len(mild_negative_indices)),
    )

    fig, ax = plt.subplots(figsize=(CURVE_FIG_WIDTH, CURVE_FIG_HEIGHT), dpi=220)
    for curve, is_hidden in zip(negative_display, negative_hidden_mask):
        if not is_hidden:
            ax.plot(cycles, curve, color="#0B3D91", alpha=0.24, linewidth=0.62, solid_capstyle="round", zorder=1)

    for curve, is_hidden in zip(positive_display, positive_hidden_mask):
        if not is_hidden:
            ax.plot(cycles, curve, color="#E31A1C", alpha=0.55, linewidth=1.0, solid_capstyle="round", zorder=2)

    for index in mild_negative_indices:
        ax.plot(cycles, negative_display[index], color="#CFCFCF", alpha=0.82, linewidth=1.15, solid_capstyle="round", zorder=3)

    for index in mild_positive_indices:
        ax.plot(cycles, positive_display[index], color="#CFCFCF", alpha=0.82, linewidth=1.15, solid_capstyle="round", zorder=3)
    ...
    ax.grid(True, color="#B0B0B0", alpha=0.28, linewidth=0.8)
    plt.tight_layout()
    plt.savefig(representative_outlier_path, bbox_inches="tight", dpi=300)
    plt.close(fig)

    save_combined_curve_summary(
        summary_path,
        negative_wells,
        positive_wells,
        negative_distances,
        ...
        negative_outliers,
        positive_outliers,
        negative_early_outliers,
        positive_early_outliers,
        negative_late_drop_outliers,
        negative_late_drop_scores,
    )

    print("[6] Combined positive/negative curves finished")
    print(f"    Negative curves: {len(negative_wells)}; outliers: {int(np.sum(negative_outliers))}; threshold={negative_threshold:.4f}")
    print(f"    Positive curves: {len(positive_wells)}; outliers: {int(np.sum(positive_outliers))}; threshold={positive_threshold:.4f}")
    ...
    print(f"    Hidden early negative curves: {int(np.sum(negative_early_outliers))}; threshold={negative_early_threshold:.4f}")
    print(f"    Hidden early positive curves: {int(np.sum(positive_early_outliers))}; threshold={positive_early_threshold:.4f}")
    print(f"    Late-drop negative filter: {LATE_DROP_FILTER_ENABLED}; hide when any cycle after {LATE_DROP_BASELINE_CYCLES} falls below that curve's first-{LATE_DROP_BASELINE_CYCLES}-cycle mean")
    print(f"    Hidden late-drop negative curves: {int(np.sum(negative_late_drop_outliers))}; threshold={negative_late_drop_threshold:.4f}")
    print(f"    Hide outlier curves: {COMBINED_HIDE_OUTLIER_CURVES}")
    print(f"    Positive display smooth window: {POSITIVE_COMBINED_SMOOTH_WINDOW}; shrink factor={POSITIVE_COMBINED_SHRINK_FACTOR:.2f}")
    print(f"    Negative display smooth window: {NEGATIVE_COMBINED_SMOOTH_WINDOW}; shrink factor={NEGATIVE_COMBINED_SHRINK_FACTOR:.2f}")
    rezero_applied = COMBINED_REZERO_AFTER_SMOOTH and CURVE_VALUE_MODE in {"delta", "normalized"}
    print(f"    Re-zero after smoothing: {rezero_applied}; cycles={COMBINED_REZERO_CYCLES}")
    print(f"    Combined figure: {figure_path}")
    print(f"    Mild-outlier figure: {representative_outlier_path}")
    print(f"    Outlier summary: {summary_path}")


def run_workflow(method):
    ...
    if not valid_wells:
        raise ValueError("No final valid wells were found; endpoint classification cannot continue.")
    ...
    if method == "kmeans":
        result = classify_by_kmeans(endpoint_gray, endpoint_color, valid_wells)
    elif method == "threshold":
        result = classify_by_threshold(
            endpoint_gray,
            endpoint_color,
            valid_wells,
    ...
    plot_negative_well_curves(curve_corrected_paths, result["negative_wells"])
    plot_positive_well_curves(curve_corrected_paths, result["positive_wells"])
    plot_combined_well_curves(curve_corrected_paths, result["negative_wells"], result["positive_wells"])
    print("\n>>> Workflow completed.")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Crop original images, filter wells, correct illumination, and classify endpoint dPCR wells."
    )
    ...
    parser.add_argument(
        ...
        help="Endpoint classification method. kmeans references linchuang.py; threshold references threshold.py.",
    )
    parser.add_argument("--positive-threshold", type=float, default=POSITIVE_THRESHOLD)
    parser.add_argument(
        "--disable-endpoint-spatial-correction",
        action="store_true",
        help="Disable endpoint intensity spatial detrending before kmeans classification.",
    )
    parser.add_argument("--endpoint-spatial-grid-cols", type=int, default=ENDPOINT_SPATIAL_GRID_COLS)
    parser.add_argument("--endpoint-spatial-grid-rows", type=int, default=ENDPOINT_SPATIAL_GRID_ROWS)
    parser.add_argument(
        "--endpoint-spatial-background-quantile",
        type=float,
        default=ENDPOINT_SPATIAL_BACKGROUND_QUANTILE,
    )
    ...
    parser.add_argument(
        ...
        help="Hide curve outliers in the combined positive/negative curve figure.",
    )
    ...
    parser.add_argument("--negative-combined-smooth-window", type=int, default=NEGATIVE_COMBINED_SMOOTH_WINDOW)
    parser.add_argument(
        "--negative-combined-shrink-factor",
        type=float,
        default=NEGATIVE_COMBINED_SHRINK_FACTOR,
        help="Display-only shrink factor for non-outlier negative curves in the combined figure. 1=no shrink, 0=median curve.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Curve y-value mode: normalized=DeltaF/F0, delta=gray value change, raw=corrected gray value, sum_delta=integrated gray value change, early_max_zero=raw minus a shared plotting offset.",
    )
    parser.add_argument(
        "--early-max-zero-offset",
        type=float,
        default=EARLY_MAX_ZERO_OFFSET,
        help="Value subtracted from raw curves when --curve-value-mode early_max_zero. If omitted, use the shared early-cycle maximum.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--raw-threshold",
        action="store_true",
        help="Use raw endpoint intensity for threshold mode instead of normalized intensity.",
    )
...
if __name__ == "__main__":
    ...
    ENDPOINT_SPATIAL_CORRECTION = not args.disable_endpoint_spatial_correction
    ENDPOINT_SPATIAL_GRID_COLS = max(2, args.endpoint_spatial_grid_cols)
    ENDPOINT_SPATIAL_GRID_ROWS = max(2, args.endpoint_spatial_grid_rows)
    ENDPOINT_SPATIAL_BACKGROUND_QUANTILE = min(
        0.49, max(0.01, args.endpoint_spatial_background_quantile)
    )
    THRESHOLD_IS_NORMALIZED = not args.raw_threshold
    ...
    NEGATIVE_COMBINED_SMOOTH_WINDOW = args.negative_combined_smooth_window
    NEGATIVE_COMBINED_SHRINK_FACTOR = args.negative_combined_shrink_factor
    ...
    EARLY_MAX_ZERO_OFFSET = args.early_max_zero_offset
